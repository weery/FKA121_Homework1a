...
int main()
{
    ...
    // ******************************************************
    // Equilibrate temperature and pressure to target values
    // ******************************************************

    for (int equil = 0; equil < 2; equil++) {

        double target_temp = temperature_eq[equil];
    ...
    // *******************************************************
    // Then equilibrate only temperature to target +- delta T
    // and measure the average energy to compute difference
    // *******************************************************
    for (int d = 0; d < 2; d++) {
        ...
        for (int i = 0; i < nbr_of_timesteps_eq; i++) {
            ...
            /* Update isntantaneous temperature and pressure */
            inst_temperature_eq = instantaneous_temperature(energy_kin_eq, nbr_of_particles);
            inst_pressure_eq = instantaneous_pressure(virial_eq, inst_temperature_eq,
                nbr_of_particles, volume);
