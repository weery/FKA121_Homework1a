...
\begin{table}[h!]
	...
	\begin{tabular}{l|cc}
		...
		$C_V / (\unit{eV/kg\,K})$ (kinetic) & $ 6.2949 \cdot 10^{-2}$ & $ 8.2569 \cdot 10^{-2}$ \\
		$C_V / (\unit{eV/kg\,K})$ (potential) & $ 6.4307 \cdot 10^{-2}$ & $ 8.4425 \cdot 10^{-2}$ \\ \hline
	\end{tabular}
	\label{tab:prob5}
\end{table}
...
$8.2569/6.2949=1.3117$.



\section*{Problem 6}

It is also possible to compute the heat capacity from the relation
\begin{equation}
...
With Matlab we find that the first local minimum is localized at $r_m = 3.7797$, giving us the coordination number $I(r_m) \approx 12.2279
$. The expected coordination number for an fcc structure is 12 \cite{al_coordination_nbr}, but our result is slightly higher. This is expected since the system is melted and the particles move around more freely. If the system was solid we would expect a result closer to 12.
