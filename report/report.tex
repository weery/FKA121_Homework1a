...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{\textit{Upper left:} Time evolution of the pressure during equilibration. The target pressure was set to 1 atm. \textit{Upper right:} The time evolution of the temperature during equilibration. The target temperature was set to 500$^\circ$ C and we can see that it quickly reaches the target temperature. \textit{Lower:} Trajectories for three of the aluminium particles during 100 ps after the equilibration process. As we can see they stay close to the initial position, which indicates that the system is in its solid state.}
